...
const getLocation = resp => ((resp) ? ((resp.request) ? resp.request.href.split('?')[0] : resp.responseURL.split('?')[0]) : '');
...
function samlRequest(r, html, resp) {
  ...
  if (samlData) {
    const options = { url: samlData.url, form: { SAMLRequest: samlData.payload } };
...
function samlResponse(r, html, data = {}) {
  ...
  if (samlData) {
    ...
    const options = { url: samlData.url, form: { SAMLResponse: samlData.payload } };
